...
from app.services.vehicle_observations import search_observations, upsert_observation
...
def test_vehicle_observation_upserts_one_track_and_searches_exact_attributes(db):
    ...
    payload = search_observations(
        ...
        vehicle_type="car", vehicle_color="white", limit=10,
    )
...
def test_investigation_and_recognition_setting_apis(monkeypatch, tmp_path):
    ...
    with client:
        ...
        assert "identity is not confirmed" in result.json()["disclaimer"]
